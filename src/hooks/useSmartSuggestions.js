...
export default function useSmartSuggestions(messages, currentIntent) {
  const suggestions = useMemo(() => {
    ...
    // Try to extract unit from last user message
    let unit = ''
    const unitMatch = lastText.match(/unit\s+(\d+[^\s]*)/i)
    if (unitMatch) {
    ...
    if (lastText.includes('parking')) {
      // Replace {unit} in suggestions if possible
      return CONTEXT_SUGGESTIONS.parking.map(q => q.replace('{unit}', unit || '___'))
    }

    // Use intent-based suggestions
    if (currentIntent === 'property_query') {
      return CONTEXT_SUGGESTIONS.property_query.map(q => q.replace('{unit}', unit || '___'))
    }
